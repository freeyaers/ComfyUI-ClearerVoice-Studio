...
import sys, time, os, tqdm, torch, argparse, glob, subprocess, warnings, cv2, pickle, pdb, math, python_speech_features, uuid
import numpy as np
...
def process_tse(args, model, device, data_reader, output_wave_dir):
	...
	with torch.no_grad():
		for videoPath in data_reader:  # Loop over all video samples
			# Use timestamp + UUID to create a unique English folder name
			timestamp = int(time.time())
			unique_id = str(uuid.uuid4())[:8]
			video_args.savePath = f'{output_wave_dir}/video_{timestamp}_{unique_id}/'
...
def args_param():
    ...
    # Use parse_known_args to ignore unknown arguments (like --windows-standalone-build from ComfyUI)
    video_args, _ = parser.parse_known_args()
    return video_args
...
def main(video_args, args):
    ...
    for idx, file in enumerate(files):
        orig_file = f"{file[:-9]}orig_{idx}.mp4"
        est_wav_file = f"{file[:-9]}est_{idx}.wav"
        est_mp4_file = f"{file[:-9]}est_{idx}.mp4"
        
        # Step 1: Convert AVI to MP4
        command = f"ffmpeg -y -i {file} {orig_file} -loglevel panic"
        output1 = subprocess.call(command, shell=True, stdout=None)
        
        if output1 == 0:
            # Step 2: Combine video and audio
            command = f"ffmpeg -y -i {orig_file} -i {est_wav_file} -c:v copy -map 0:v:0 -map 1:a:0 -shortest {est_mp4_file} -loglevel panic"
            output2 = subprocess.call(command, shell=True, stdout=None)
            
            if output2 == 0:
                # Step 3: Remove temporary files
                temp_files = [file, est_wav_file, orig_file]
...
def inference_video(video_args):
	...
	sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Starting face detection...\r\n")
	...
	for fidx, fname in enumerate(flist):
		try:
			...
			if image is None:
				# Skip if image is None (couldn't be read)
				sys.stderr.write(f"Warning: Could not read image {fname}\r\n")
				dets.append([])
				continue
			...
			# Skip if there's an error processing the image
			sys.stderr.write(f"Warning: Error processing image {fname}: {e}\r\n")
			dets.append([])
			continue
...
def visualization(tracks, est_sources, video_args):
	...
	for tidx, track in enumerate(tracks):
		...
		command = "ffmpeg -y -i %s -c:v libx264 -c:a aac -movflags +faststart %s -loglevel panic" % (
				os.path.join(video_args.pyaviPath, 'video_out_%s.avi' % tidx),
				os.path.join(video_args.pyaviPath, 'video_est_%s.mp4' % tidx)
			)
		output = subprocess.call(command, shell=True, stdout=None)
		
		# Wait for the file to be fully written before checking
		time.sleep(0.5)
		
		# Remove the temporary AVI file after successful conversion
		if output == 0 and os.path.exists(os.path.join(video_args.pyaviPath, 'video_est_%s.mp4' % tidx)):
			temp_avi_file = os.path.join(video_args.pyaviPath, 'video_out_%s.avi' % tidx)
			if os.path.exists(temp_avi_file):
	...
	# Remove temporary files after successful conversion
	if output == 0:
		temp_files = [
			os.path.join(video_args.pyaviPath, 'video_only.avi'),
			os.path.join(video_args.pyaviPath, 'video.avi'),
			os.path.join(video_args.pyaviPath, 'audio.wav')
		]
		for temp_file in temp_files:
			if os.path.exists(temp_file):
				os.remove(temp_file)
