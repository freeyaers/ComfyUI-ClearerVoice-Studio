...
class SpeechModel:
    ...
    def __init__(self, args):
        ...
        # Check if a GPU is available (includes both CUDA and ROCm)
        elif torch.cuda.is_available():
            try:
                # For ROCm or CUDA, just use the first available GPU
                args.use_cuda = 1
                self.device = torch.device('cuda')
                print("GPU detected, using first available GPU")
            except Exception as e:
                # If any error occurs, use CPU
                args.use_cuda = 0
                self.device = torch.device('cpu')
                print(f"Error detecting GPU, using CPU: {e}")
        else:
